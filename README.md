...
![Fableseed shown on different devices.](./docs/screenshots/amiresponsive.png)
...
* GSAP animations weren't firing on mobile phones (actual mobiles, not the responsive mobile breakpoint). I had to change the relative position parameters to absolute and add a ScrollTrigger refresh after time timeline compnents. Also changed split.lines to the id of the actual element where it was setting visibility. Works fine across devices now.
...
* Extremely last minute, I realised I hadn't properly styled my site for screens 2560px and above. I spent a mad 4 hours+ sorting it out. Problem was the highest breakpoint for Tailwind was 2xl which applied at 1536px. This meant any upsizing/responsive reflow of content looked terrible on my own 1900px screen. I ended up having to create a tailwind.config.js file, which has to sit in the project root for Tailwind to find it. This let me define my own custom breakpoint of 2560px for 2xl and left everything else alone which looked lovely. I had some trouble with the Django allauth forms, but ChatGPT and [this tailwind article ](https://tailwindcss.com/docs/functions-and-directives#apply-directive) helped me figure out how to apply tailwind styles inline using @apply in my tailwind input file, called tailwind.css. I was able to use this to also style labels on other forms that were styled by Crispy, where the Crispy forms layout helper wouldn't touch them no matter what I did. 
